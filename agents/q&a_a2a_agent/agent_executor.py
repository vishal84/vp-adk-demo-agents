...
from a2a.types import Part, Role, TaskState, TextPart, UnsupportedOperationError
...
class QnAAgentExecutor(AgentExecutor):
    ...
    async def execute(
        ...
        if self.runner is None:
            raise RuntimeError("Runner failed to initialize")
        ...
        if context.task_id is None or context.context_id is None:
            raise ValueError("Task or context identifier is missing from request context")
        ...
        try:
            ...
            async for event in runner.run_async(
                ...
                if event.is_final_response():
                    ...
                    answer_parts = cast(list[Part], [TextPart(text=answer)])
