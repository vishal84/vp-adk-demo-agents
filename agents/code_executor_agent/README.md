...
```
uv run adk deploy agent_engine --project="gsi-gemini-ent" --region="us-central1" --requirements_file=./agent/requirements.txt --env_file=".env" --trace_to_cloud  --staging_bucket="gs://ge-agent-staging" --display_name="Code Executor Agent" --description="An agent that can execute python code and draw charts" ./agent```
