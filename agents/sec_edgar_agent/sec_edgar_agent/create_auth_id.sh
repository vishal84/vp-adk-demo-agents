...
required_vars=("GOOGLE_CLOUD_PROJECT" "AUTH_ID" "AUTH_URI" "TOKEN_URI" "CLIENT_ID" "CLIENT_SECRET" "SCOPES")
...
echo "🚀 Creating Gemini Enterprise Authorization..."
...
AUTHORIZATION_URI="${AUTH_URI}?&scope=${ENCODED_SCOPES}&include_granted_scopes=true&response_type=code&access_type=offline&prompt=consent"
...
if [ "$http_code" -eq 200 ] || [ "$http_code" -eq 201 ]; then
    ...
    echo "   Auth ID: $AUTH_ID"
    echo "   Next step: Run 'scripts/create_ge_agent.sh' to create the agent"
else
