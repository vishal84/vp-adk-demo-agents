...
# def whoami(tool_context: ToolContext):
#     creds = Credentials.from_authorized_user_info(
#         tool_context.state["oauth_tool_tokens"], SCOPES
#     )
...
#     tool_context.state['_user_email'] = user_email
...
#     if "oauth_tool_tokens" in tool_context.state:
#         creds = Credentials.from_authorized_user_info(
#             tool_context.state["oauth_tool_tokens"], SCOPES
#         )
...
#         else:
#             print("Need User Authorization to access your email address.")
#             auth_scheme = OAuth2(
#                 flows=OAuthFlows(
#                     authorizationCode=OAuthFlowAuthorizationCode(
#                         authorizationUrl=f"{OAUTH_AUTH_URL}",
#                         tokenUrl=f"{OAUTH_TOKEN_URL}",
#                         scopes=SCOPES,
#                     )
#                 )
#             )
...
#                 return "Need User Authorization to access your email address. " \
...
#         tool_context.state["oauth_tool_tokens"] = json.loads(creds.to_json())
...
#     After authentication use your tool `whoami` to output the user's email address. ONLY call `whoami` if the user is already authenticated.
#     If the user is already authenticated, you should confirm their email address and state that they are already logged in.
#     """,
#     tools=[auth_user, whoami],
# )

# # --- Initialize ADK App ---
# # This 'app' object is the main entry point for your agent
# # when deployed to the ADK Web Dev UI.
